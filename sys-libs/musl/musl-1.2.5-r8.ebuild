...
SRC_URI+="
	...
	https://dev.gentoo.org/~blueness/musl-misc/iconv.c
        mimalloc? ( https://github.com/microsoft/mimalloc/archive/refs/tags/v${MIMALLOC_VER}.tar.gz )
"
...
src_prepare() {
	...
	cp -r "${WORKDIR}/mimalloc-${MIMALLOC_VER}" "${S}/mimalloc" || die
		eapply "${FILESDIR}"/0001-implement-necessary-bits-for-musl-integration.patch
		eapply "${FILESDIR}"/0001-plumb-in-support-for-externally-provided-allocator-l.patch
		eapply "${FILESDIR}"/mimalloc-errno.patch
		eapply "${FILESDIR}"/mimalloc-tweak-options.patch
		cp "${FILESDIR}"/mimalloc-verify-syms.sh "${S}"/
		cp "${FILESDIR}"/mimalloc.c  "${S}"/mimalloc/src
	fi
...
src_compile() {
	...
	emake $(usev mimalloc "EXTRA_OBJ=${S}/src/malloc/external/mimalloc.o")
	if ! is_crosspkg ; then
